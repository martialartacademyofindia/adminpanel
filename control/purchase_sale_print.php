...
$dd_products = "";
if ($id != 0) {
    $arr_products_key_value = get_product_as_array(1, '', "Purchase");
...
if ($page_type == "Purchase") {
    ...
    $data_arr_input = array();
    $data_arr_input['select_field'] = " CONCAT(pro_name,'-',po1.po_name,'-',po2.po_name) as mname , CONCAT(invpro_id,'##',pro_id,'##',invpro_po_id,'##',invpro_po_id_2,'##',invpro_final_pro_price,'##',invpro_pro_qty) as mid, inv_purchase_invoice_no  ";
    $data_arr_input['select_field'] = " CONCAT(pro_name,'-',po1.po_name,'-',po2.po_name) as mname , CONCAT(invpro_id,'##',pro_id,'##',invpro_po_id,'##',invpro_po_id_2) as mid ,inv_purchase_invoice_no ";
...
<?php echo $to_address; ?>
                                                    ...
                                                    <!-- /.col -->
                                                    <div class="col-sm-4 invoice-col">
                                                        <b>Invoice No.</b> <?php echo $inv_purchase_invoice_no; ?><br>
...
if ($res_invoice_products["errormsg"] == "" && $res_invoice_products["count"] > 0) {
    ...
    //    $nproduct_name =  selected_value_return($arr_products_key_value, $products_row['invpro_id_purchase'] . '##' . $products_row['invpro_pro_id'] . '##' . $products_row['invpro_po_id'] . '##' . $products_row['invpro_po_id_2']); 
                                                    ...
                                                    <table class="table">
                                                        <tbody>
                                                            <tr>
                                                                ...
                                                                <td><?php echo $inv_total_amount-$inv_cgst_amount - $inv_sgst_amount; ?></span> INR</td>
                                                            </tr>
                                                            <tr>
