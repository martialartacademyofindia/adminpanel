...
// $condtion_enrolled = $condtion_non_enrolled = "";
// backup query for following before changing it.
/*
$stuent_non_enrolled_query =" SELECT ".$select_f." FROM sm_student
INNER JOIN sm_student_course
ON (sc_stu_id = stu_id )
INNER JOIN sm_belt ON (sc_be_id = be_id  AND sc_is_current =1 )
INNER JOIN sm_branch_type
ON (sc_brt_id = brt_id )
LEFT JOIN sm_course ON ( co_id = sc_co_id)
LEFT JOIN sm_exam_student_entrolled ON (stu_id = exs_stu_id AND exs_ex_id  = ".$ex_id.")
LEFT JOIN
...
WHERE exs_id IS NULL AND (stu_status = 'A' OR (stu_status = 'I' AND (DATEDIFF(now(),stu_deactivation_date) < 30) )) " ;
*/
...
ON (sc_brt_id = brt_id )
LEFT JOIN sm_course ON ( co_id = sc_co_id)
LEFT JOIN sm_exam_student_entrolled ON (stu_id = exs_stu_id AND sc_be_id = exs_be_id AND sc_co_id = exs_co_id  )
LEFT JOIN
(SELECT SUM(IF(eca_total_marks IS NULL,0,eca_total_marks)) as  eca_total_marks , be_id
FROM sm_belt LEFT JOIN sm_exam_categories_allocation ON (be_id =  eca_be_id)
GROUP BY be_id ) as M ON (sc_be_id = M.be_id)
WHERE (exs_id IS NULL OR  (exs_result_status = 'F' OR exs_result_status = 'A' ) )  AND (stu_status = 'A' OR (stu_status = 'I' AND (DATEDIFF(now(),stu_deactivation_date) < 30) )) " ;
...
/*
$mfinalquery = "SELECT  exs_ex_id, be_name_for, exs_result_status,exs_result_marks,exs_enroll_next,	sc_joined_date , stu_br_id, M.be_id, exs_total_marks, exs_fee, exs_paid,  co_name, M.eca_total_marks , be_belt_exam_fee, exs_fee,exs_paid, IF(exs_id IS NULL,0,exs_id) as exs_id,  co_name, DATEDIFF(now(),sc_joined_date) n_j_diff , be_belt_duration , stu_gr_no,stu_first_name,stu_phone,stu_email,stu_status,stu_id,stu_middle_name,stu_last_name,brt_name,be_name_for be_name FROM  sm_student
INNER JOIN sm_student_course
ON (sc_stu_id = stu_id )
INNER JOIN sm_belt ON (sc_be_id = be_id  AND sc_is_current =1  )
INNER JOIN sm_branch_type
ON (sc_brt_id = brt_id )
LEFT JOIN sm_course ON ( co_id = sc_co_id)
 LEFT  JOIN sm_exam_student_entrolled ON (stu_id = exs_stu_id AND exs_ex_id  = 1)
LEFT JOIN
(SELECT SUM(IF(eca_total_marks IS NULL,0,eca_total_marks)) as  eca_total_marks , be_id
FROM sm_belt LEFT JOIN sm_exam_categories_allocation ON (be_id =  eca_be_id)
GROUP BY be_id ) as M ON (sc_be_id = M.be_id)
WHERE  exs_id IS NOT NULL and stu_br_id= 1 AND (stu_status = 'A' OR (stu_status = 'I' AND (DATEDIFF(now(),stu_deactivation_date) < 30) ))
UNION
SELECT  exs_ex_id, be_name_for, exs_result_status,exs_result_marks,exs_enroll_next,	sc_joined_date , stu_br_id, M.be_id, exs_total_marks, exs_fee, exs_paid,  co_name, M.eca_total_marks , be_belt_exam_fee, exs_fee,exs_paid, IF(exs_id IS NULL,0,exs_id) as exs_id,  co_name, DATEDIFF(now(),sc_joined_date) n_j_diff , be_belt_duration , stu_gr_no,stu_first_name,stu_phone,stu_email,stu_status,stu_id,stu_middle_name,stu_last_name,brt_name,be_name_for be_name FROM  sm_student
INNER JOIN sm_student_course
ON (sc_stu_id = stu_id )
INNER JOIN sm_belt ON (sc_be_id = be_id  AND sc_is_current =1  )
INNER JOIN sm_branch_type
...
FROM sm_belt LEFT JOIN sm_exam_categories_allocation ON (be_id =  eca_be_id)
GROUP BY be_id ) as M ON (sc_be_id = M.be_id)
WHERE exs_id IS NULL AND DATEDIFF(\"2017-03-26\",sc_joined_date) >= be_belt_duration   and stu_br_id= 1 AND (stu_status = 'A' OR (stu_status = 'I' AND (DATEDIFF(now(),stu_deactivation_date) < 30) ))  order by stu_gr_no asc";
*/
...
if (  (isset($pay_fee) && $pay_fee == 1  ) || (  isset($addresult) && $addresult == 1  ) )
{
  ...
  $pageObj = new PS_Pagination($table, $select_f, $condition1." ".$condition. " ".$condition2, $per_page, 10, "per_page=" . $per_page . "&stu_first_name=" . $stu_first_name . "&order by=" . $order_by . "&order=" . $order);
}
else if (isset($enroll) && $enroll == 1  )
{
// echo $condition1 ; exit(0);
  $student_entroll_query = $student_entroll_query . $condition;
  $stuent_non_enrolled_query = $stuent_non_enrolled_query . $condition." AND ".$condition1. $condition2;
