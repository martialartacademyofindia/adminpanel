...
include("../includes/class/gallery_photos.php");
...
$caption = "Add Gallery";
$btn_caption = "Add Gallery";
if ($id != 0) {
    $caption = "Edit Gallery";
    $btn_caption = "Edit Gallery";
}
// Set Page Title
$page_title = $caption;
...
if ($act == 'add') {
    ...
    $arr_duplicate_school_name = found_duplicate('sm_gallery', 'ga_title', $ga_title, $not_value);
...
if ($errormsg == '') {
    if ($_FILES['ga_cover_image']['error'] == 0) {  /// Image
        ...
        if (!(in_array($file_ext, $arr_allow_file_type) == 1)) {
            ...
            $ga_cover_image = "gallery_" . strtolower(randomPrefix(5)) . "_" . clean_name($_FILES['ga_cover_image']['name']);
            $DestPath = GALLERY_IMAGE . $ga_cover_image;
    ...
    if ($errormsg == '') {
        ...
        $sm_gallery->data["ga_title"]=$ga_title;
            $sm_gallery->data["ga_cover_image"]=SITE_URL . "images/gallery/" . $ga_cover_image;
        ...
        if ($result['status'] == 'failure') {
            ...
            while ($iimg <= $file_counter) {
                if ($_FILES['gallery_details_pic_' . $iimg]['error'] == 0) {  /// Image
                    ...
                    if (!(in_array($file_ext, $arr_allow_file_type) == 1)) {
                        ...
                        $gp_image = "gallery_" . strtolower(randomPrefix(5)) . "_" . clean_name($_FILES['gallery_details_pic_' . $iimg]['name']);
                        $DestPath = GALLERY_IMAGE . $gp_image;
                        move_uploaded_file($_FILES['gallery_details_pic_' . $iimg]['tmp_name'], $DestPath);
                        //echo $gp_image; exit;
                        // adding files in db
                        // $sm_gallery_photos->data["gp_id"]=$gp_id;
                        $sm_gallery_photos = new gallery_photos();

                        $sm_gallery_photos->data["gp_image"] = SITE_URL . "images/gallery/" . $gp_image;
            ...
            if ($errormsg == '') {
                ...
                if ($ga_status == 'A')
                {
                    ...
                    $arr_data["not_goToScreen"]="gallery";
                    ...
                    $arr_data["gcmp_goToScreen"]="gallery";

                    add_notes($arr_data);

                }

                header('Location:manage_gallery.php?msg=2&page=1&per_page=' . $per_page);
                exit(0);
            }
        }
    }
...
if ($act == 'update') {
    ...
    $arr_duplicate_gallery_name = found_duplicate('sm_gallery', 'ga_title', $ga_title, $not_value);
...
if ($errormsg == '') {
    if ($_FILES['ga_cover_image']['error'] == 0) {  /// Image
        ...
        if (!(in_array($file_ext, $arr_allow_file_type) == 1)) {
            ...
            $DestPath = GALLERY_IMAGE . $ga_cover_image;
            move_uploaded_file($_FILES['ga_cover_image']['tmp_name'], $DestPath);
            $ga_cover_image = SITE_URL . "/images/gallery/" . $ga_cover_image;
        }
    } else {
    ...
    if ($errormsg == '') {
        ...
        if ($result['status'] == 'failure') {
            ...
            if ($errormsg == '') {
                ...
                header('Location:manage_gallery.php?msg=3&page=1&per_page=' . $per_page);
                exit(0);
            }
        }
    }
...
if ($id != 0) {
    $q = "SELECT gp.gp_image ,gp.gp_id  FROM  sm_gallery_photos gp INNER JOIN sm_gallery g ON (gp.gp_ga_id=g.ga_id) INNER JOIN sm_school_master sm ON (g.ga_sc_id=sm.sc_id) WHERE g.ga_id= " . $id;
                                    ...
                                    <div class="box-body">
                                        <div class=" col-md-6">
                                            <div class="form-group">
                                                ...
                                                <div class="col-sm-9">
                                                    <input required type="text" name="ga_title" id="ga_title"  placeholder="Gallery Name" value="<?php echo $ga_title; ?>" class="form-control" />
                                                </div>
                                            </div>
                                            <div class="form-group">
        ...
        ?>
                                                            <tr class="<?php echo $class; ?>" id="tr_<?php echo $db_row['gp_id']; ?>" >
                                                                ...
                                                                <td>
                                                                    ...
                                                                    <center><a href="javascript:void(0);" id="a_process_<?php echo $db_row['gp_id']; ?>" class="delete glyphicon glyphicon-remove" onclick="delete_image('delete-gallery-image',<?php echo $db_row['gp_id']; ?>)"></a></center></td>
                                                            </tr>
                                                            <?php
                                                        }
                                                        ?>
