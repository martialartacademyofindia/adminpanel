...
include("../includes/class/faculty_document.php");
...
$doc_fac_id = get_rdata('doc_fac_id');
$doc_br_id = get_rdata('doc_br_id',$tmp_admin_id);
...
$caption = "Faculty Document";
$btn_caption = "Faculty Document";

//if ($id != 0) 
//{
//    $caption = "Edit Faculty Document";
//    $btn_caption = "Edit Faculty Document";
//}
...
$msg = get_rdata('msg', '');
if (isset($msg) && $msg == 1) {
    $successmsg = "Faculty Document Has Been Deleted Successfully";
} else if (isset($msg) && $msg == 2) {
    $successmsg = "Faculty Document Has Been Added Successfully";
} else if (isset($msg) && $msg == 3) {
    $successmsg = "Faculty Document Has Been Updated Successfully";
} else {
    $successmsg = '';
}
...
if (($act == 'update' || $act == 'delete') && $doc_fac_id != 0 && $id != 0) {
   
    $cquery = "DELETE FROM sm_faculty_document WHERE doc_br_id = ".$doc_br_id." AND  doc_id = " . $id . " AND doc_fac_id=" . $doc_fac_id;
    ...
    if ($errormsg == '') {
        ...
        if ($_FILES['doc_file_name_original']['error'] == 0) {  /// Image
            ...
            if (!(in_array($file_ext, $arr_allow_file_type_document) == 1)) {
                ...
                $DestPath = FACULTY_DOCUMENT_IMAGE . $doc_file_name_save;
        ...
        if ($errormsg == '') {
            $faculty_documentmaster = new faculty_document();
            $faculty_documentmaster->data["doc_name"] = $doc_name;
            $faculty_documentmaster->data["doc_fac_id"] = $doc_fac_id;
            $faculty_documentmaster->data["doc_br_id"] = $doc_br_id;
            $faculty_documentmaster->data["doc_file_name_original"] = $doc_file_name_original;
            $faculty_documentmaster->data["doc_file_name_save"] = $doc_file_name_save;
            $faculty_documentmaster->data["doc_create_date"] = $doc_create_date;
            $faculty_documentmaster->data["doc_create_by_id"] = $doc_create_by_id;
            $faculty_documentmaster->data["doc_update_date"] = $doc_update_date;
            $faculty_documentmaster->data["doc_update_by_id"] = $doc_update_by_id;

            $faculty_documentmaster->action = 'insert';
            $result = $faculty_documentmaster->process();
            if ($result['status'] == 'failure') {
                $errormsg = $result['errormsg'];
            } else {
                header('Location:faculty_document.php?doc_fac_id='.$doc_fac_id.'&msg=2&page=1&per_page=' . $per_page);
                exit(0);
            }
        }
    }
...
$q = "SELECT * FROM sm_faculty_document WHERE doc_fac_id = " . $doc_fac_id;
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="box box-info">
                                ...
                                <form name="form1" id="form1" enctype="multipart/form-data" method="post" class="form-horizontal" onsubmit="return validate_add_edit_circular();">
                                    ...
                                    <input type="hidden" id="doc_fac_id" name="doc_fac_id" value="<?php echo $doc_fac_id; ?>" />
                                    <div class="box-body">
                                        <div class=" col-md-6">
                                                ...
                                                foreach ($result["res"] as $arr_db) {
                                                    ...
                                                    <tr class="<?php echo $class; ?>">
                                                        ...
                                                        <td style="padding-left:10px;">
                                                            <a target="_blank" href="<?php echo FACULTY_DOCUMENT_IMAGE . $arr_db["doc_file_name_save"]; ?>" class="text-info" ><?php echo ($arr_db["doc_name"] != "") ? $arr_db["doc_name"] : $arr_db["doc_file_name_original"]; ?></a></td>                                                        
