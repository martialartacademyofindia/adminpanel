...
$btn_caption = "Add Customer";
if ($id != 0) {
    $caption = "Edit Customer";
    $btn_caption = "Edit Customer";
}
// Set Page Title
$page_title = $caption;
...
if ($act == 'add') {
    if ($del_company_name !='')
    {
        ...
        $arr_duplicate_name = found_duplicate('sm_customer', 'del_company_name', escape($del_company_name), $not_value);
        if ($arr_duplicate_name['error_message'] != '') {
            $errormsg = $arr_duplicate_name['error_message'];
        } else if ($arr_duplicate_name['duplicate'] == true) {
            $errormsg = 'Duplicate entry for customer name';
        }
    }
    ...
    if ($errormsg == '') {
        ...
        if ($result['status'] == 'failure') {
            $errormsg = $result['errormsg'];
                  ...
                  header('Location:manage_customer.php?msg=2&page=1&per_page=' . $per_page);
                  exit(0);
         }
    }
}
    ...
    if ($del_company_name !='')
    {
        ...
        if ($result['status'] == 'failure') {
            $errormsg = $result['errormsg'];
                  ...
                  header('Location:manage_customer.php?msg=3&page=1&per_page=' . $per_page);
                exit(0);
