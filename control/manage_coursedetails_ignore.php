...
$cd_status = get_rdata('cd_status');
$cd_eu_duration = get_rdata('cd_eu_duration');
$cd_nu_duration = get_rdata('cd_nu_duration');
$cd_eu_fee = get_rdata('cd_eu_fee');
$cd_nu_fee = get_rdata('cd_nu_fee');
$cd_eu_exam_fee = get_rdata('cd_eu_exam_fee');
$cd_nu_exam_fee = get_rdata('cd_nu_exam_fee');
$cd_eu_fee_onemonth = get_rdata('cd_eu_fee_onemonth');
$cd_nu_fee_onemonth = get_rdata('cd_nu_fee_onemonth');
...
/*
    ...
    if ($arr_check_delete["error_message"] != '') {
        ...
        $errormsg = "Delete process can not be completed because of found Course Details reference with student";
    }
    */
    /*
    if ($errormsg == '') {
...
$pageObj = new PS_Pagination($table, '*', "$condition", $per_page, 10, "per_page=" . $per_page . "&cd_name=" . $cd_name . "&order by=" . $order_by . "&order=" . $order);
                                                            ...
                                                            <table id="example2" class="table table-bordered table-hover">
                                                                <thead>
                                                                    <tr>
                                                                        ...
                                                                        <th align="center"><a href="manage_branch.php?page=<?php echo $page; ?>&per_page=<?php echo $per_page; ?>&order_by=cd_name&order=<?php echo $order; ?>">Name <span class="<?php echo $cd_name_arrow; ?>"></span></a></th>
    ...
    for ($i = 1; $db_row = $objData->fetch(); $i++) {
        ...
        ?>
                                                                        ...
                                                                        <td style="padding-left:10px;"><?php echo ($db_row['br_status']=='A'?'Active':'Inactive'); ?></td>
                                                                                <td><center>
                                                                                        <a href="add_edit_branch.php?id=<?php echo $db_row['br_id']; ?>&per_page=<?php echo $per_page; ?>" class="edit glyphicon glyphicon-pencil" ></a>&nbsp;
                                                                                        <a href="javascript:void(0);" class="delete glyphicon glyphicon-remove" data-toggle="modal" data-target="#ConfirmDelete" onclick="delete_record(<?php echo $db_row['br_id']; ?>, 'Course Details')"></a></center></td>
                                                                            </tr>
                                                                            <?php
                                                                        }
                                                                    } else {
