...
$ne_status = get_rdata('ne_status');

// Set success message based on msg ID
$msg = get_rdata('msg', '');
if (isset($msg) && $msg == 1) {
    $successmsg = "News Has Been Deleted Successfully";
} else if (isset($msg) && $msg == 2) {
    $successmsg = "News Has Been Added Successfully";
} else if (isset($msg) && $msg == 3) {
    $successmsg = "News Has Been Updated Successfully";
} else {
    $successmsg = '';
}
...
$order_by = get_rdata('order_by', 'ne_id');
$order = get_rdata('order', 'asc');
$client_arrow = $ne_title_arrow = $ne_title_arrow = 'glyphicon glyphicon-chevron-down';
if ($order == 'asc') {
    $order = 'desc';
    if ($order_by == 'ne_title') {
...
if ($act == 'delete' && $id != 0) {
    ...
    $sm_news_master = new news();
    $sm_news_master->action = 'delete';
    $del_where = "ne_id = " . $id;
    if (session_get('admin_login_type') == 'school') {
        $del_where .= " and ne_sc_id= " . session_get('admin_sc_id');
    }
    $sm_news_master->where = $del_where;
    $result = $sm_news_master->process();
...
$condition = '1';
if ($ne_title != '') {
    $condition .= " and 	ne_title LIKE '%" . $ne_title . "%'";
}
...
$table = "sm_news INNER JOIN sm_school_master ON (sc_id=ne_sc_id)";
// echo "SELECT * FROM ".$table. " WHERE " .$condition;
$pageObj = new PS_Pagination($table, '*', "$condition", $per_page, 10, "per_page=" . $per_page . "&ne_title=" . $ne_title . "&order by=" . $order_by . "&order=" . $order);
                            ...
                            <form class="form-horizontal" name="form1" id="form1" method="post"
                                ...
                                <div class="box-body">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label class="col-sm-3 control-label">News Name</label>
                                            <div class="col-sm-9">
                                                <input type="text" name="ne_title" id="ne_title"
                                                    placeholder="Enter News Name" value="<?php echo $ne_title; ?>"
                                                    class="form-control" />
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="box-footer">
                                    ...
                                    <button type="button" class="btn btn-default"
                                        onclick="window.location.href = 'manage_news.php'">Cancel</button>
                                </div><!-- /.box-footer -->
                            </form>
                        </div>
                                ...
                                <table id="example2" class="table table-bordered table-hover">
                                    <thead>
                                        <tr>
                                            ...
                                            <th align="center"><a
                                                    href="manage_news.php?page=<?php echo $page; ?>&per_page=<?php echo $per_page; ?>&order_by=ne_title&order=<?php echo $order; ?>">News
                                                    Name <span class="<?php echo $ne_title_arrow; ?>"></span></a></th>
    ...
    for ($i = 1; $db_row = $objData->fetch(); $i++) {
        ...
        ?>
                                        <tr class="<?php echo $class; ?>">
                                            ...
                                            <td style="padding-left:10px;"><?php echo $db_row['ne_title']; ?></td>
                                            <td style="padding-left:10px;">
                                                <?php echo ($db_row['ne_status'] == 'A' ? 'Active' : 'Inactive'); ?>
                                            </td>
                                            <td>
                                                <center><a
                                                        href="add_edit_news.php?id=<?php echo $db_row['ne_id']; ?>&per_page=<?php echo $per_page; ?>"
                                                        class="edit glyphicon glyphicon-pencil"></a>&nbsp;
                                                    <a href="javascript:void(0);" data-toggle="modal"
                                                        data-target="#ConfirmDelete"
                                                        class="delete glyphicon glyphicon-remove"
                                                        onclick="delete_record(<?php echo $db_row['ne_id']; ?>,'News')"></a>
                                                </center>
                                            </td>
                                        </tr>
                                        <?php
