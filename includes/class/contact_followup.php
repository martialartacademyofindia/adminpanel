<?php
class contact_followup
{
	public $table = 'sm_contact_followup';
	public $action = '';
	public $parameters = '';
	public $cquery = '';
	public $data =  array() ;
	...
	function process()
	{
		...
		if ($this->action=='update' || $this->action=='delete' || $this->action=='get')
		{
			...
			if ($condition == '' && $this->process_id !=0)
				$condition = "con_id=".$this->process_id;
 				  if ($condition == '' && ($this->action=='update' || $this->action=='delete' ))
				  {
					  return array("errormsg"=>"Server Communication Error: 031","id"=>0,"status"=>"failure");
				  }
		}
